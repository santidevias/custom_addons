...
class ProductProduct(models.Model):
    ...
    def _compute_display_name(self):
        ...
        for product in self:
            if product.equipment_rent_ok:
                product.display_name = _("%s (Equipment Rental)", product.display_name)

    def _get_qty_in_rent_domain(self):
        return [
            ('equipment_is_rental', '=', True),
    ...
    def _get_qty_in_equipment_rent(self):
        ...
        active_rental_lines = self.env['sale.order.line']._read_group(
            ...
            groupby=['product_id'],
            aggregates=['qty_delivered:sum', 'qty_equipment_returned:sum'],
        )
        res = {product.id: qty_delivered - qty_equipment_returned for product, qty_delivered, qty_equipment_returned in active_rental_lines}
        for product in self:
            product.qty_in_equipment_rent = res.get(product.id, 0)
    ...
    def _compute_delay_price(self, duration):
        ...
        return days * self.equipment_extra_daily + hours * self.equipment_extra_hourly
    ...
    def _get_best_pricing_rule(self, **kwargs):
        ...
        :rtype: product.equipment.pricing
        """
