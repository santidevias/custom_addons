...
class Pricelist(models.Model):
    ...
    equipment_product_pricing_ids = fields.One2many(
        ...
        string="Equipment Renting Price Rules",
        domain=['|', ('product_template_id', '=', None), ('product_template_id.active', '=', True)],
    )

    @api.constrains('equipment_product_pricing_ids')
    def _check_pricing_product_rental(self):
        for pricing in self.equipment_product_pricing_ids:
            if not pricing.product_template_id.equipment_rent_ok:
                raise UserError(_(
                    "You can not have a time-based rule for products that are not rentable."
                ))

    def _compute_price_rule(
        self, products, quantity, currency=None, date=False, equipment_start_date=None, end_date=None,
        **kwargs
    ):
        ...
        results = {}
        if self._enable_rental_price(equipment_start_date, end_date):
            rental_products = products.filtered('equipment_rent_ok')
            Pricing = self.env['product.equipment.pricing']
            for product in rental_products:
                if equipment_start_date and end_date:
                    pricing = product._get_best_pricing_rule(
                        equipment_start_date=equipment_start_date, end_date=end_date, pricelist=self, currency=currency
                    )
                    duration_vals = Pricing._compute_duration_vals(equipment_start_date, end_date)
