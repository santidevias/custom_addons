...
class SaleOrderLine(models.Model):
    ...
    # Stored because a product could have been equipment_rent_ok when added to the SO but then updated
    equipment_is_rental = fields.Boolean(compute='_compute_equipment_is_rental', store=True, precompute=True)

    qty_equipment_returned = fields.Float("Equipment Returned", default=0.0, copy=False)
    equipment_start_date = fields.Datetime(related='order_id.equipment_rental_start_date')
    equipment_return_date = fields.Datetime(related='order_id.equipment_rental_return_date')
    equipment_reservation_begin = fields.Datetime(
        string="Pickup date - padding time (Equipment)", compute='_compute_equipment_reservation_begin', store=True)

    is_product_equipment_rentable = fields.Boolean(related='product_id.equipment_rent_ok', depends=['product_id'])

    @api.depends('order_id.equipment_rental_start_date')
    def _compute_equipment_reservation_begin(self):
        lines = self.filtered('equipment_is_rental')
        for line in lines:
            line.equipment_reservation_begin = line.order_id.equipment_rental_start_date
        (self - lines).equipment_reservation_begin = None
    ...
    @api.depends('equipment_is_rental')
    def _compute_qty_delivered_method(self):
        """Allow modification of delivered qty without depending on stock moves."""
        rental_lines = self.filtered('equipment_is_rental')
        super(SaleOrderLine, self - rental_lines)._compute_qty_delivered_method()
        rental_lines.qty_delivered_method = 'manual'

    @api.depends('order_id.equipment_rental_start_date', 'order_id.equipment_rental_return_date', 'equipment_is_rental')
    ...
    def _compute_equipment_is_rental(self):
        for line in self:
            line.equipment_is_rental = line.is_product_equipment_rentable and line.env.context.get('in_equipment_rental_app')
    ...
    _sql_constraints = [
        ('rental_stock_coherence',
            "CHECK(NOT equipment_is_rental OR qty_equipment_returned <= qty_delivered)",
    ...
    def _get_rental_order_line_description(self):
        tz = self._get_tz()
        equipment_start_date = self.order_id.equipment_rental_start_date
        equipment_return_date = self.order_id.equipment_rental_return_date
        env = self.with_context(use_babel=True).env
        if equipment_start_date and equipment_return_date\
           and equipment_start_date.replace(tzinfo=UTC).astimezone(timezone(tz)).date()\
               == equipment_return_date.replace(tzinfo=UTC).astimezone(timezone(tz)).date():
            # If return day is the same as pickup day, don't display equipment_return_date Y/M/D in description.
            return_date_part = format_time(env, equipment_return_date, tz=tz, time_format=False)
        else:
            return_date_part = format_datetime(env, equipment_return_date, tz=tz, dt_format=False)
        start_date_part = format_datetime(env, equipment_start_date, tz=tz, dt_format=False)
        return _(
    ...
    def _generate_delay_line(self, qty_equipment_returned):
        """Generate a sale order line representing the delay cost due to the late return.

        :param float qty_equipment_returned: returned quantity
        """
        self.ensure_one()

        self = self.with_company(self.company_id)
        duration = fields.Datetime.now() - self.equipment_return_date
        ...
        delay_product = self.company_id.equipment_extra_product
        if not delay_product:
            ...
            self.company_id.equipment_extra_product = delay_product
        ...
        order_line_vals = self._prepare_delay_line_vals(delay_product, delay_price * qty_equipment_returned)
    ...
    def _get_delay_line_description(self):
        ...
        tz = self._get_tz()
        env = self.with_context(use_babel=True).env
        expected_date = format_datetime(env, self.equipment_return_date, tz=tz, dt_format=False)
    ...
    def _get_pricelist_price(self):
        ...
        The displayed price will only be the price given by the product.equipment.pricing rules matching the
        ...
        if self.equipment_is_rental:
            ...
            return self.order_id.pricelist_id._get_product_price(
                ...
                equipment_start_date=self.equipment_start_date,
                end_date=self.equipment_return_date,
            )
    ...
    def _lines_without_price_recomputation(self):
        ...
        return res.filtered(lambda l: not l.equipment_is_rental)
