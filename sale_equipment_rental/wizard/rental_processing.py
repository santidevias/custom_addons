...
class RentalProcessing(models.TransientModel):
    _name = 'equipment.rental.order.wizard'
    _description = "Pick-up/Return products"

    order_id = fields.Many2one('sale.order', required=True, ondelete='cascade')
    rental_wizard_line_ids = fields.One2many('equipment.rental.order.wizard.line', 'rental_order_wizard_id')
    ...
    equipment_is_late = fields.Boolean(related='order_id.equipment_is_late')
    ...
    def _get_wizard_lines(self):
        ...
        if rental_lines_to_process:
            ...
            for line in rental_lines_to_process:
                lines_values.append(self.env['equipment.rental.order.wizard.line']._default_wizard_line_vals(line, self.status))
...
class RentalProcessingLine(models.TransientModel):
    _name = 'equipment.rental.order.wizard.line'
    ...
    def _default_wizard_line_vals(self, line, status):
        return {
            ...
            'qty_equipment_returned': line.qty_equipment_returned if status == 'pickup' else line.qty_delivered - line.qty_equipment_returned,
        }

    rental_order_wizard_id = fields.Many2one('equipment.rental.order.wizard', 'Rental Order Wizard', required=True, ondelete='cascade')
    ...
    qty_equipment_returned = fields.Float("Returned")

    @api.constrains('qty_equipment_returned', 'qty_delivered')
    def _only_pickedup_can_be_returned(self):
        for wizard_line in self:
            if wizard_line.status == 'return' and wizard_line.qty_equipment_returned > wizard_line.qty_delivered:
    ...
    def _apply(self):
        ...
        for wizard_line in self:
            ...
            elif wizard_line.status == 'return' and wizard_line.qty_equipment_returned > 0:
                if wizard_line.rental_order_wizard_id.equipment_is_late:
                    # Delays facturation
                    order_line._generate_delay_line(wizard_line.qty_equipment_returned)

                order_line.update({
                    'qty_equipment_returned': order_line.qty_equipment_returned + wizard_line.qty_equipment_returned
                })
        return msg
    ...
    def _get_diff(self):
        ...
        if self.status == 'pickup':
            ...
            return self.qty_equipment_returned, order_line.qty_equipment_returned, order_line.qty_equipment_returned + self.qty_equipment_returned
