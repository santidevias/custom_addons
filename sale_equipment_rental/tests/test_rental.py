...
class TestRentalCommon(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.product_id = cls.env['product.product'].create({
            ...
            'type': 'consu',
            'equipment_rent_ok': True,
            'equipment_extra_hourly': 7.0,
            'equipment_extra_daily': 30.0,
        })

        cls.product_template_id = cls.product_id.product_tmpl_id

        cls.product_template_id.equipment_product_pricing_ids.unlink()
        cls.recurrence_hourly = cls.env['sale.temporal.recurrence.equipment'].create({'duration': 1.0, 'unit': 'hour'})
        cls.recurrence_5_hours = cls.env['sale.temporal.recurrence.equipment'].create({'duration': 5.0, 'unit': 'hour'})
        cls.recurrence_15_hours = cls.env['sale.temporal.recurrence.equipment'].create({'duration': 15.0, 'unit': 'hour'})
        cls.recurrence_daily = cls.env['sale.temporal.recurrence.equipment'].create({'duration': 1.0, 'unit': 'day'})
        ...
        for pricing in PRICINGS:
            ...
            cls.env['product.equipment.pricing'].create(pricing)
    ...
    def test_pricing(self):
        ...
        self.assertEqual(
            self.product_id._get_best_pricing_rule(
                equipment_start_date=now, end_date=now + relativedelta(hours=9)
        ...
        self.assertEqual(
            self.product_id._get_best_pricing_rule(
                equipment_start_date=now, end_date=now + relativedelta(hours=1)
        ...
        self.assertEqual(
            self.product_id._get_best_pricing_rule(
                equipment_start_date=now, end_date=now + relativedelta(hours=16)
        ...
        self.assertEqual(
            self.product_id._get_best_pricing_rule(
                equipment_start_date=now, end_date=now + relativedelta(hours=20)
        ...
        self.assertEqual(
            self.product_id._get_best_pricing_rule(
                equipment_start_date=now, end_date=now + relativedelta(hours=24)
    ...
    def test_pricelists(self):
        ...
        self.product_template_id.equipment_product_pricing_ids.unlink()
        for pricing in PRICINGS:
            pricing.update(product_template_id=self.product_template_id.id)
            pricing = self.env['product.equipment.pricing'].create(pricing)

        equipment_reservation_begin = fields.Datetime.now()
        pickup_date = equipment_reservation_begin + relativedelta(days=1)
        equipment_return_date = pickup_date + relativedelta(hours=1)

        sale_order = self.env['sale.order'].create({
            'partner_id': partner.id,
            'equipment_rental_start_date': pickup_date,
        ...
        sol = self.env['sale.order.line'].create({
            ...
            'equipment_reservation_begin': equipment_reservation_begin,
        })
        sol.update({'equipment_is_rental': True})
    ...
    def test_delay_pricing(self):
        ...
        self.product_id.equipment_extra_hourly = 2.5
        self.product_id.equipment_extra_daily = 15.0
    ...
    def test_no_pickup_nor_return(self):
        ...
        recurrence_hour = self.env['sale.temporal.recurrence.equipment'].create({'duration': 1.0, 'unit': 'hour'})
        PRICINGS = [
            {
    ...
    def test_no_price_update_on_pickup_return_update(self):
        ...
        self.assertEqual(sol.price_unit, 1, "No pricing should be taken into account if no pickup nor return date.")
        sale_order.write({
            'equipment_rental_start_date': fields.Datetime.now() + relativedelta(days=1),
            'equipment_rental_return_date': fields.Datetime.now() + relativedelta(days=1, hours=1),
        })
        sol.write({'equipment_is_rental': True})
        self.assertEqual(sol.price_unit, 1, "Update price should not alter first computed price.")
        sale_order._recompute_prices()
    ...
    def test_no_pricing_for_pricelist(self):
        ...
        for pricing in PRICINGS:
            pricing.update(product_template_id=self.product_template_id.id)
            self.env['product.equipment.pricing'].create(pricing)
    ...
    def test_contextual_price_is_pickup_return_dependant(self):
        ...
        price = self.product_template_id.with_context(
            equipment_start_date=fields.Datetime.now() + relativedelta(days=1),
    ...
    def test_discount_on_sol_remains(self):
        ...
        sale_order = self.env['sale.order'].create({
            'partner_id': partner.id,
            'equipment_rental_start_date': now,
            'equipment_rental_return_date': one_day_later,
        })
        ...
        sol = self.env['sale.order.line'].create({
            ...
            'price_unit': 10,
            'order_id': sale_order.id,
            'equipment_reservation_begin': now,
            'discount': discount,
        })
        sol.update({'equipment_is_rental': True})
        ...
        pickup_wizard = self.env['equipment.rental.order.wizard'].with_context(
            action_dict['context']
        ).create({})
    ...
    def test_renting_taxes_inc2ex(self):
        ...
        self.assertEqual(sale_order.equipment_duration_days, 1, 'Default duration should be one day')
        self.assertEqual(sale_order.equipment_remaining_hours, 0, 'Default duration should be one day')
