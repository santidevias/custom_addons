...
from odoo.addons.sale_equipment_rental.models.sale_order import EQUIPMENT_RENTAL_STATUS


class RentalSchedule(models.Model):
    _name = "sale.equipment.rental.schedule"
    ...
    def _read_group_product_ids(self, products, domain, order):
        if self._context.get('restrict_renting_products'):
            return products
        all_rental_products = products.search([('equipment_rent_ok', '=', True)], order=order)
    ...
    equipment_return_date = fields.Datetime('Return Date', readonly=True)
    ...
    qty_equipment_returned = fields.Float('Qty Returned', readonly=True)
    ...
    equipment_rental_status = fields.Selection(selection=EQUIPMENT_RENTAL_STATUS, string="Rental Status", readonly=True)
    ...
    def _quantity(self):
        return """
            ...
            sum(sol.qty_equipment_returned / u.factor * u2.factor) as qty_equipment_returned
        """

    def _late(self):
        return """
            CASE WHEN sol.state != 'sale' THEN FALSE
                WHEN s.equipment_rental_start_date < NOW() AT TIME ZONE 'UTC' AND sol.qty_delivered < sol.product_uom_qty THEN TRUE
                WHEN s.equipment_rental_return_date < NOW() AT TIME ZONE 'UTC' AND sol.qty_equipment_returned < sol.qty_delivered THEN TRUE
            ELSE FALSE
            END as late
        """
    ...
    def _report_line_status(self):
        return """
            CASE WHEN sol.qty_equipment_returned = sol.qty_delivered
    ...
    def _color(self):
        ...
        return """
            CASE WHEN s.equipment_rental_start_date < NOW() AT TIME ZONE 'UTC' AND sol.qty_delivered < sol.product_uom_qty THEN 4
                WHEN s.equipment_rental_return_date < NOW() AT TIME ZONE 'UTC' AND sol.qty_equipment_returned < sol.qty_delivered THEN 6
                WHEN sol.qty_equipment_returned = sol.qty_delivered AND sol.qty_delivered = sol.product_uom_qty THEN 7
    ...
    def _select(self):
        return """%s,
            ...
            s.equipment_rental_start_date as pickup_date,
            s.equipment_rental_return_date as equipment_return_date,
            s.state as state,
            s.equipment_rental_status as equipment_rental_status,
            s.partner_id as partner_id,
            s.user_id as user_id,
            s.company_id as company_id,
            extract(epoch from avg(date_trunc('day',s.equipment_rental_return_date)-date_trunc('day',s.equipment_rental_start_date)))/(24*60*60)::decimal(16,2) as delay,
    ...
    def _groupby(self):
        return """
            ...
            s.name,
            s.date_order,
            s.equipment_rental_start_date,
            s.equipment_rental_return_date,
            s.partner_id,
            s.user_id,
            s.equipment_rental_status,
            s.company_id,
            s.pricelist_id,
