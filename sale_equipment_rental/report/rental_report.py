...
class RentalReport(models.Model):
    _name = "sale.equipment.rental.report"
    ...
    qty_equipment_returned = fields.Float('Daily Returned Qty', readonly=True)
    ...
    def _quantity(self):
        return """
            ...
            sol.qty_equipment_returned / (u.factor * u2.factor) AS qty_equipment_returned
        """

    def _price(self):
        return """
            sol.price_subtotal / (date_part('day',so.equipment_rental_return_date - so.equipment_rental_start_date) + 1)
        """
    ...
    def _select(self):
        return """
            ...
            pt.categ_id,
            p.product_tmpl_id,
            generate_series(so.equipment_rental_start_date::date, so.equipment_rental_return_date::date, '1 day'::interval)::date date,
            %s AS price,
            sol.company_id,
            sol.state,
            sol.currency_id
